...
class taobao_image_loader(BaseDataLoader):
    """
...
transform = T.Compose([
    ...
    T.Normalize([0.5,0.5,0.5], [0.5,0.5,0.5]),  # 标准化至[-1, 1]，规定均值和标准差
])
...
class Train_Dataset(torch.utils.data.Dataset):
    def __init__(self,path):
        ...
        # 获取图片总数
        self.img_data = []
        for idx,dir in enumerate(classes):
            files = os.listdir(os.path.join(path,dir))
            for file in files:
                self.img_data.append([idx,os.path.join(dir,file)])
    ...
    def __getitem__(self, index):
        ...
        if not os.path.isfile(os.path.join(self.root,self.img_data[index][1])):
            print(self.img_data[index])
            print('file not exist')
            exit()
        pil_img = Image.open(os.path.join(self.root,self.img_data[index][1]))
        pil_img = transform(pil_img)
        label = int(self.img_data[index][0])
